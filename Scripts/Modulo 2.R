...
# ej. peso, altura, presión arterial, puntaje, tiempo

# Variable independiente: categórica
# ej. sexo, tratamiento, grupo A / grupo B

# La prueba estadistica compara la media de una variable continua entre grupos.

# -------------------------------

# K = 2

# Significa que hay 2 grupos
#  ej. hombres vs mujeres
#  tratamiento vs control

# Independientes: Los grupos no están relacionados entre sí una persona solo pertenece a un grupo

# n ≥ 30: El tamaño de muestra es al menos 30 observaciones
...
# T de Student (independientes, varianzas homogéneas)

t.test(colesterol ~ dieta, data = datos, var.equal = TRUE) # TRUE para T de student
...
t.test(colesterol ~ dieta, data = datos, var.equal = FALSE) # FALSE para Test de Welch
...
# Si p < 0.05, se rechaza H₀ → hay evidencia de diferencia significativa de que la dieta influye en los niveles de 
# colesterol, aun cuando las varianzas son distintas.
...
# En este caso, esperarías que el grupo con dieta Alta en Grasas tenga en promedio un colesterol mayor y con más 
# variabilidad que el grupo Balanceada.
...
library("ggplot2") # Llamar a la libreria para cargar sus funciones
...
# p ≤ 0.05 → Se rechaza la normalidad (los datos no parecen normales).

# 2. Verificar homogeneidad de varianzas
bartlett.test(colesterol ~ dieta, data = datos) # p>0.05 → Homogeneidad
...
anova_result <- ezANOVA(data = datos,
                        dv = valor, # Variable dependiente
                        wid = id, # Identificador de los sujetos
                        within = momento, # La variable que cambia dentro del mismo individuo
                        detailed = TRUE)
anova_result

# $ANOVA

# (Intercept): No lo interpretes directamente, porque solo indica que el promedio general de tus mediciones (valor) 
# no es cero. Siempre suele ser significativo.

# momento: p = 0.3455 → el valor p.
# p<.05 → vacío (no hay asterisco), así que no es significativo.

# Interpretación:
# No hay evidencia estadísticamente significativa de que el valor promedio cambie entre los diferentes momentos 
# medidos.

# $`Mauchly's Test for Sphericity`

# Este test comprueba la esfericidad, un supuesto del ANOVA de medidas repetidas (que las varianzas de las diferencias
# entre condiciones sean iguales).

# p = 0.116 > 0.05 → se cumple el supuesto de esfericidad.
# (no hay violación → no necesitas correcciones)
...
# Estas son correcciones del valor p (Greenhouse-Geisser (GG) y Huynh-Feldt (HF)), que solo se usan si Mauchly es 
# significativo (es decir, si hay violación de esfericidad).
# En este caso debes usar el valor de p corregido (p[GG] o p[HF]) para decidir si el efecto es significativo.
...
chisq.test(tabla) # Hay asociación significativa entre tipo de dieta y mejoría.
...
fisher.test(tabla) # Hay asociación significativa entre tipo de tratamiento y mejoría.
...
ggplot(datos, aes(x = Tratamiento, y = Frecuencia, fill = Resultado)) +
  geom_bar(stat = "identity", position = "stack") + # Aquí las alturas de las barras serán los valores de la columna Frecuencia. "stack" → apila las barras una encima de otra (por defecto).
